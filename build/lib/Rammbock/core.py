...
from Rammbock.synchronization import SynchronizedType
from Rammbock.templates.containers import BagTemplate, CaseTemplate
from Rammbock.message import _StructuredElement
from Rammbock.networking import (TCPServer, TCPClient, UDPServer, UDPClient, SCTPServer,
                         SCTPClient, _NamedCache)
from Rammbock.message_sequence import MessageSequence
from Rammbock.templates import (Protocol, UInt, Int, PDU, MessageTemplate, Char, Binary,
                        TBCD, StructTemplate, ListTemplate, UnionTemplate,
                        BinaryContainerTemplate, ConditionalTemplate,
                        TBCDContainerTemplate)
from Rammbock.binary_tools import to_0xhex, to_bin
...
class RammbockCore(object):
    ...
    def reset_rammbock(self):
        ...
        reuse faster.
        """
        for client in list(self._clients._cache.values()):
            client.close()
        for server in list(self._servers._cache.values()):
            server.close()
        self._init_caches()
    ...
    def server_sends_binary(self, message, name=None, connection=None, label=None):
        ...
        server.send(message.encode(), alias=connection)
    ...
    def client_receives_binary(self, name=None, timeout=None, label=None):
        ...
        """
        client, name = self._clients.get_with_name(name)
        msg = client.receive(timeout=timeout).decode('utf8')
    ...
    def server_receives_binary(self, name=None, timeout=None, connection=None, label=None):
        ...
        """
        return self.server_receives_binary_from(name, timeout, connection=connection, label=label)[0].decode('utf8')
    ...
    def save_template(self, name, unlocked=False):
        ...
        """
        if isinstance(unlocked, six.string_types):
    ...
    def _send_message(self, callback, parameters):
        ...
        callback(msg, label=self._current_container.name, **configs)
