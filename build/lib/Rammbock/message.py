...
from Rammbock.binary_tools import to_0xhex, to_binary_string_of_length, \
...
class _StructuredElement(object):
    ...
    def _get_raw_bytes(self):
        return ''.join((field._raw.decode('utf8') for field in self._fields.values()))
...
class Field(object):
    ...
    @property
    def _raw(self):
        return self._original_value.ljust(self._length, '\x00'.encode())
