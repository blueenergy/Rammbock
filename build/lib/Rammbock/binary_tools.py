...
def _int_to_bin(integer):
    ...
    return LONGLONG.pack(integer).lstrip('\x00'.encode()) or '\x00'
...
def to_bin_of_length(length, string_value):
    ...
    if len(bin) > length:
        raise AssertionError('Too long binary value %s (max length %d)'
                             % (string_value, length))
    return bin.rjust(length, '\x00'.encode())


def to_hex(binary):
    return binascii.hexlify(binary.encode())


def to_0xhex(binary):
    return '0x'.encode() + to_hex(binary)
